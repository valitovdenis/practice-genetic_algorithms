...
class App:
    ...
    def update_nav_buttons(self):
        ...
        if idx < total - 1:
            ...
            if self.controller.history[-1]['generation'] < self.controller.max_gen:
                self.btn_forward.config(state=tk.NORMAL)
                self.btn_end.config(state=tk.NORMAL)
            else:
    ...
    def update_display(self):
        ...
        self.update_plot(state['generation'], state['best_cost'])
    ...
    def on_end(self):
        current_state = self.controller.get_current_state()
        while current_state['generation'] < self.params['gen']:
            self.controller.step_forward()
            current_state = self.controller.get_current_state()
            self.update_display()
